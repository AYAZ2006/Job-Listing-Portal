...
export default function Jobs() {
  ...
  useEffect(() => {axios.get("https://job-listing-portal-8.onrender.com/jobs/").then((res) => {setJobs(res.data.map(job => ({ ...job, is_favorite: job.is_favorite || false })));}).catch((err) => {toast.error("Failed to load jobs");}).finally(() => setLoading(false));}, []);
  ...
  const toggleFavorite = async (jobId) => {
    try {
      const res = await axios.post("https://job-listing-portal-8.onrender.com/favorite/", {job_id: jobId,email: email});
