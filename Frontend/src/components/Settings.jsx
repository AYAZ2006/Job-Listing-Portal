...
export default function Settings() {
  ...
  useEffect(() => {
    let score = 0;
    const totalItems = 13;
    if (profile.name?.trim() && profile.surname?.trim()) score += 1;
    if (profile.email) score += 1;
    if (profile.mobile?.trim()) score += 1;
    if (profile.gender) score += 1;
    if (resume) score += 1;
    if (profile.category && profile.qualification?.trim()) score += 1;
    if (profile.course_start && profile.course_end) score += 1;
    if (profile.career_status) score += 1;
    if (skills.length >= 3) score += 1;
    if (experiences.length > 0) score += 1;
    if (profile.about?.trim().length >= 20) score += 1;
    if (profile.linkedin?.trim()) score += 1;
    if (profile.portfolio?.trim()) score += 1;
    const percentage = Math.round((score / totalItems) * 100);
    setCompletionPercentage(percentage);
    localStorage.setItem("profileCompletion", percentage.toString());
  }, [profile, skills, experiences, resume]);
  ...
  useEffect(() => {
    ...
    const handleSave = () => {
      ...
      .then(async res => {
          const data = await res.json();
          if (!res.ok) { toast.error("Backend error"); return; }
  ...
  return (
    <>
      ...
      <div className="h-screen w-full bg-[#0e0e0e] text-gray-200 flex">
        <aside className="w-80 h-screen sticky top-0 p-8 overflow-y-auto hide-scrollbar bg-[#1a1a1a] border-r border-white/10 hidden md:block">
          <h2 className="text-xl font-bold text-white mb-6 ml-10 mt-20">Profile Completion</h2>
          <div className="flex flex-col items-center mb-8">
            <div className="relative w-36 h-36">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="45" stroke="#2a2a2a" strokeWidth="10" fill="none" />
                <circle cx="50" cy="50" r="45" stroke={completionPercentage >= 90 ? "#4ade80" : completionPercentage >= 70 ? "#10b981" : completionPercentage >= 40 ? "#f59e0b" : "#ef4444"} strokeWidth="10" fill="none" strokeDasharray="283" strokeDashoffset={283 - (283 * completionPercentage) / 100} strokeLinecap="round" className="transition-all duration-1000 ease-out"/>
              </svg>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-4xl font-bold text-white">{completionPercentage}<span className="text-2xl">%</span></span>
              </div>
            </div>
            <p className="mt-4 text-sm font-medium text-gray-300">
              {completionPercentage < 50 && "Let's complete your profile!"}
              {completionPercentage >= 50 && completionPercentage < 90 && "Great progress!"}
              {completionPercentage >= 90 && "Amazing! You're all set!"}
            </p>
          </div>
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">What's Complete?</h3>
            {[
              { label: "Name", filled: !!profile.name && !!profile.surname },
              { label: "Email", filled: !!profile.email },
              { label: "Mobile Number", filled: !!profile.mobile },
              { label: "Gender", filled: !!profile.gender },
              { label: "Resume", filled: !!resume },
              { label: "Category & Qualification", filled: !!profile.category && !!profile.qualification },
              { label: "Course Duration", filled: !!profile.course_start && !!profile.course_end },
              { label: "Career Status", filled: !!profile.career_status },
              { label: "Skills", filled: skills.length >= 3 },
              { label: "Experience", filled: experiences.length > 0 },
              { label: "About Me", filled: !!profile.about && profile.about.length > 20 },
              { label: "LinkedIn", filled: !!profile.linkedin },
              { label: "Portfolio / GitHub", filled: !!profile.portfolio },
            ].map((item, index) => (
              <div key={index} className={`flex items-center justify-between p-3 rounded-lg border transition-all duration-300 ${item.filled ? "bg-green-900/20 border-green-500/30" : "bg-red-900/10 border-red-500/20"}`}>
          ...
          <div className="mt-8 p-4 bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-500/20 rounded-xl">
            <p className="text-xs text-gray-300 text-center">Profiles with <span className="font-bold text-white">90%+ completion</span> get{" "}<span className="text-yellow-400">3x more recruiter views</span></p>
          </div>
        </aside>
              ...
              <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
                <div className="bg-[#151515] rounded-2xl border border-red-600/30 p-8 max-w-md mx-4 shadow-2xl">
                  ...
                  <div className="flex gap-4 justify-end">
                    <button onClick={() => setShowModal(false)} className="px-6 py-3 bg-black/50 cursor-pointer text-gray-300 rounded-lg hover:bg-black transition">Cancel</button>
                    <button onClick={() => {handleDelete();setShowModal(false);}}className="px-6 py-3 cursor-pointer bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-medium">Yes, Delete My Account</button>
                  </div>
                </div>
              </div>
            )}
