...
function Manage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const res = await axios.get(
        `http://127.0.0.1:8000/my-jobs/?username=${username}`
      );
      setJobs(res.data);
    } catch (err) {
      console.log(err);
  ...
  const fetchInternships = async () => {
    try {
      const username = localStorage.getItem("username");
      const res = await axios.get(
        `http://127.0.0.1:8000/my-internships/?username=${username}`
      );
            ...
            <div key={job.id} className="bg-black/40 border border-white/10 rounded-2xl p-5 flex flex-col md:flex-row gap-5">
                ...
                <div className="relative">
                    ...
                    {openMenu === job.id && (
                        ...
                        <button className="w-full text-left py-1 hover:bg-white/10">Edit</button>
                        <button className="w-full text-left py-1 hover:bg-white/10">Delete</button>
                        </div>
                    )}
                </div>
            </div>
        ...
        {view === "internship" && (
          <div className="flex flex-col gap-4 mt-5 text-white">
            ...
            {internships.map((intern) => (
              <div
                key={intern.id}
                className="bg-black/40 border border-white/10 rounded-xl p-4 flex justify-between items-start"
              >
                <div>
                ...
                <div className="relative">
                  <button onClick={() => setOpenMenu(openMenu === intern.id ? null : intern.id)}>
                    <MoreVertical />
                  </button>
            ...
            <button
              className="mt-3 bg-gray-600 hover:bg-gray-700 text-white p-3 rounded-lg cursor-pointer"
