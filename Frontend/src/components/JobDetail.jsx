          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 sm:gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
              ...
              <div className="text-center sm:text-left">
                <h1 className="text-2xl sm:text-xl font-bold text-white">{job.job_title}</h1>
                <p className="text-base sm:text-sm text-gray-400">{job.company_name}</p>
              </div>
            </div>
            <div className="w-full sm:w-auto flex justify-center sm:justify-end">
              <button className="w-full sm:w-auto px-8 py-3 sm:px-6 sm:py-2 rounded-lg font-semibold bg-[linear-gradient(180deg,#2a2a2a,#141414)] text-white border border-white/10 shadow cursor-pointer hover:shadow-lg transition-shadow text-lg sm:text-base" onClick={() => {if (profileCompletion < 90) {toast.error(
